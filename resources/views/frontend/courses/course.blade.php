                ...
                <div class="col-md-9">
                    ...
                    <div class="course-details-item border-bottom-0 mb-0">
                        ...
                        <div class="course-single-text">
                            ...
                            <div class="course-details-content text-start">
                                <p>
                                    {!! $course->description !!}
                                </p>
                            </div>
                            ...
                            @if(count($lessons)  > 0)
                                <h3 class="font-weight-bold text-main text-start">
                                    <span class="">@lang('labels.frontend.course.course_timeline')</span>
                                </h3>
                            @endif
                        </div>
                    </div>
                                    ...
                                    <div class="reply-comment-box">
                                        ...
                                        <div class="teacher-faq-form">
                                            ...
                                            <form method="POST"
                                                ...
                                                <textarea class="" name="review" class="mb-2" id="review" rows="2"
                                                          cols="20">@if(isset($review)){{$review->content}} @endif</textarea>
                                                <span class="help-block text-danger">{{ $errors->first('review', ':message') }}</span>
                                                <div class="">
                                                    <button class="font-wight-bold btn-lg btn btn-primary" type="submit" value="Submit">
                                                        @lang('labels.frontend.course.add_review_now')
                                                    </button>
                                                </div>
                                            </form>
                                        </div>
                    ...
                    @if($course->bundles && (count($course->bundles) > 0))
                        <div class="course-details-category ul-li mt-5">
                            <h3 class="text-dark">@lang('labels.frontend.course.available_in_bundles')</h3>
                        </div>
                ...
                <div class="col-md-3">
                    <div class="side-bar">
                        <div class="course-side-bar-widget">
                            ...
                            @if (!$purchased_course && $matched['matched']==false && $matched['matchedBundle']==false)
                                ...
                                @if(auth()->check() && (auth()->user()->hasRole('student')) && (Cart::session(auth()->user()->id)->get( $course->id)))
                                    <button class="btn btn btn-block text-center my-2 text-uppercase  btn-success text-white bold-font"
                                ...
                                @elseif(!auth()->check())
                                    @if($course->free == 1)
                                        <a id="openLoginModal"
                                           class="btn btn-block text-white  gradient-bg text-center text-uppercase  bold-font"
                                    ...
                                    @else
                                        ...
                                        <a id="openLoginModal"
                                           class="btn btn-block my-2 bg-dark text-center text-white text-uppercase "
                                ...
                                @elseif(auth()->check() && (auth()->user()->hasRole('student')))
                                    @if($course->free == 1)
                                        <form action="{{ route('cart.getnow') }}" method="POST">
                                            ...
                                            <button class="btn btn-block text-white  gradient-bg text-center text-uppercase  bold-font"
                                    ...
                                    @else
                                        ...
                                        {{-- @if($courseInPlan==true)
                                            <a class="btn btn-block text-white  gradient-bg text-center text-uppercase  bold-font"
                                       ...
                                       {{-- <br/>
                                       <a class="btn btn-block text-white  bg-dark text-center text-uppercase  bold-font"
                                              href="{{ route('subscription.plans') }}">@lang('labels.frontend.course.subscribe')</a>
                                       @else
                                       <a class="btn btn-block text-white  gradient-bg text-center text-uppercase  bold-font"
                                          href="{{ route('subscription.plans') }}">@lang('labels.frontend.course.subscribe')</a> --}}
                            ...
                            @else
                                @if($continue_course || !empty($checkSubcribePlan))
                                    <a href="{{$continue_course != null ? route('lessons.show',['course_id' => $course->id,'slug'=>$continue_course->model->slug]) : '#'}}"
                                       class="{{$continue_course != null ? '' : 'disabled'}} btn btn-block text-white  gradient-bg text-center text-uppercase  bold-font">
                                        
                                    @if($continue_course != null)
                                        @lang('labels.frontend.course.continue_course')
                                        @else
                                        @lang('labels.frontend.course.no_lessons')
                                    @endif
                        ...
                        <div class="enrolled-student">
                            ...
                            <div class="student-number bold-font text-main">
                        ...
                        <div class="couse-feature ul-li-block">
                            <ul>
                                ...
                                <li class="d-inline-block w-100">
                                    @lang('labels.frontend.course.category') : 
                                    <span class="text-right">
                                        <a href="{{route('courses.category',['category'=>$course->category->slug])}}" target="_blank">

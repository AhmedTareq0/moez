<?php

return array (
  'backend' => 
  array (
    'access' => 
    array (
      'roles' => 
      array (
        'create' => 'انشاء دور',
        'edit' => 'تحرير الدور',
        'management' => 'إدارة الدور',
        'table' => 
        array (
          'number_of_users' => 'عدد المستخدمين',
          'permissions' => 'أذونات',
          'role' => 'الدور',
          'sort' => 'ترتيب',
          'total' => 'مجموع الأدوار',
        ),
      ),
      'users' => 
      array (
        'active' => 'المستخدمين النشطين',
        'all_permissions' => 'جميع الأذونات',
        'change_password' => 'تغيير كلمة المرور',
        'change_password_for' => 'تغيير كلمة المرور لـ :user',
        'create' => 'إنشاء مستخدم',
        'deactivated' => 'المستخدمين المعطلين',
        'deleted' => 'المستخدمين المحذوفين',
        'edit' => 'تعديل المستخدم',
        'management' => 'إدارة المستخدم',
        'no_permissions' => 'لا توجد أذونات',
        'no_roles' => 'لا توجد أدوار لتعيينها.',
        'permissions' => 'أذونات',
        'table' => 
        array (
          'abilities' => 'قدرات',
          'confirmed' => 'تم تأكيد',
          'created' => 'تم الإنشاء',
          'email' => 'البريد الإلكتروني',
          'first_name' => 'الاسم الاول',
          'id' => 'رقم الهوية',
          'last_name' => 'الاسم الاخير',
          'last_updated' => 'آخر تحديث',
          'name' => 'الاسم',
          'no_deactivated' => 'لا يوجد مستخدمين معطلين.',
          'no_deleted' => 'لا يوجد مستخدمين محذوفين',
          'other_permissions' => 'أذونات أخرى',
          'permissions' => 'أذونات',
          'roles' => 'الأدوار',
          'social' => 'اجتماعي',
          'status' => 'الحالة',
          'total' => 'مجموع المستخدمين',
        ),
        'tabs' => 
        array (
          'content' => 
          array (
            'overview' => 
            array (
              'avatar' => 'الصورة',
              'confirmed' => 'تم تأكيد',
              'created_at' => 'أنشئت في',
              'deleted_at' => 'تم الحذف في',
              'email' => 'البريد الإلكتروني',
              'first_name' => 'الاسم الاول',
              'last_login_at' => 'آخر تسجيل دخول في',
              'last_login_ip' => 'آخر تسجيل دخول IP',
              'last_name' => 'الاسم الاخير',
              'last_updated' => 'آخر تحديث',
              'name' => 'الاسم',
              'status' => 'الحالة',
              'timezone' => 'وحدة زمنية',
            ),
          ),
          'titles' => 
          array (
            'history' => 'التاريخ',
            'overview' => 'نظرة عامة',
          ),
        ),
        'user_actions' => 'إجراءات المستخدم',
        'view' => 'عرض المستخدم',
        'select_role' => 'اختر الدور',
      ),
    ),
    'backup' => 
    array (
      'api_key' => 'مفتاح API',
      'api_secret' => 'المفتاح السري ل (API)',
      'app_key' => 'مفتاح التطبيق',
      'app_secret' => 'المفتاح السري للتطبيق',
      'app_token' => 'رمز التطبيق',
      'aws' => 'AWS S3',
      'backup_files' => 'ملفات النسخ الاحتياطي',
      'backup_note' => '<b> ملاحظة </b>: لتشغيل هذه النسخة الاحتياطية بشكل صحيح ، يلزمك إضافة الكود التالي إلى  <b>CRON TAB:</b><br><code>* * * * * cd /path-to-your-project && php artisan schedule:run >> /dev/null 2>&1</code>',
      'backup_notice' => 'يرجى الرجوع الوثائق قبل بدء النسخ الاحتياطي. لديها كل التفاصيل خطوة بخطوة لإنشاء نسخة احتياطية مع Dropbox.',
      'backup_schedule' => 'جدول النسخ الاحتياطي',
      'backup_type' => 'نوع النسخ الاحتياطي',
      'bucket_name' => 'اسم الحاوية',
      'configuration' => 'الاعدادات',
      'daily' => 'يومي',
      'db' => 'قاعدة البيانات',
      'db_app' => 'ملفات قاعدة البيانات والتطبيق',
      'db_storage' => 'ملفات قاعدة البيانات والتخزين',
      'dropbox' => 'Dropbox',
      'dropbox_note' => 'يرجى التحقق من الوثائق لـ <b>كيفية الحصول على مفاتيح تطبيق Dropbox؟</b>',
      'email' => 'إشعار البريد الإلكتروني',
      'enable_disable' => 'مفعل / غير مفعل',
      'generate_backup' => 'إنشاء نسخة احتياطية',
      'monthly' => 'شهريا',
      'region' => 'منطقة',
      'title' => 'النسخ الاحتياطي',
      'weekly' => 'أسبوعي',
    ),
    'blogs' => 
    array (
      'create' => 'انشاء مدونة',
      'edit' => 'تحرير المدونة',
      'fields' => 
      array (
        'category' => 'الفئة',
        'clear' => 'مسح',
        'comments' => 'تعليقات',
        'content' => 'محتوي',
        'created' => 'تم الانشاء',
        'created_at' => 'تم إنشاؤها على',
        'featured_image' => 'صورة مميزة',
        'meta_description' => 'الوصف (Meta Description)',
        'meta_keywords' => 'الكلمات الدلالية (Meta Keywords)',
        'meta_title' => 'العنوان الوصف (Meta Title)',
        'publish' => 'نشر',
        'slug' => 'العلامة (Slug)',
        'status' => 'الحالة',
        'tags' => 'الوسوم (Tags)',
        'tags_placeholder' => 'إضافة الوسوم هنا',
        'title' => 'عنوان',
        'views' => 'المشاهدات',
      ),
      'max_file_size' => '(الحد الأقصى لحجم الملف 10 ميغابايت)',
      'title' => 'مدونة',
      'view' => 'عرض المدونات',
    ),
    'categories' => 
    array (
      'create' => 'إنشاء فئة',
      'edit' => 'تحرير الفئة',
      'fields' => 
      array (
        'courses' => 'الدورات',
        'icon' => 'أيقونة',
        'icon_type' => 
        array (
          'icon' => 'اختر أيقونة',
          'image' => 'تحميل الصور',
          'select_any' => 'اختر',
          'title' => 'نوع الأيقونة',
        ),
        'image' => 'صورة',
        'name' => 'الاسم',
        'or' => 'أو',
        'select_icon' => 'اختر أيقونة',
        'slug' => 'العلامة (Slug)',
        'status' => 'الحالة',
        'blog' => 'مدونة',
      ),
      'title' => 'الفئات',
      'view' => 'عرض الفئات',
      'not_allowed' => 'غير مسموح! الفئة تحتوي على البيانات.',
    ),
    'courses' => 
    array (
      'fields' => 
      array (
        'slug' => 'العلامة (Slug)',
        'status' => 'الحالة',
        'category' => 'الفئة',
        'course' => 'دورة',
        'course_image' => 'صورة الدورة',
        'description' => 'وصف',
        'featured' => 'متميز',
        'lessons' => 
        array (
          'add' => 'أضف دروس',
          'view' => 'عرض الدروس',
        ),
        'meta_description' => 'الوصف (Meta Description)',
        'meta_keywords' => 'كلمات دلالية (Meta Keywords)',
        'meta_title' => 'عنوان الوصف (Meta Title)',
        'popular' => 'مفضل',
        'price' => 'السعر',
        'published' => 'منشور',
        'sidebar' => 'أضف الشريط الجانبي',
        'start_date' => 'تاريخ البدء',
        'teachers' => 'مدربين',
        'title' => 'عنوان',
        'trending' => 'الشائع',
        'free' => 'مجاناً',
        'unpublished' => 'غير منشور',
        'expire_at' => 'تاريخ انتهاء/تعطيل الدورة',
        'strike' => 'السعر قبل الخصم',
      ),
      'add_categories' => 'إضافة فئات',
      'add_teachers' => 'إضافة المدربين',
      'category' => 'الفئة',
      'course_timeline' => 'الجدول الزمني للدورة',
      'create' => 'إنشاء دورة',
      'edit' => 'تحرير الدورة',
      'lesson' => 'درس',
      'listing_note' => 'سيتم عرض وترتيب الدروس المنشورة فقط.',
      'save_timeline' => 'حفظ الجدول الزمني',
      'select_category' => 'اختر الفئة',
      'select_teachers' => 'اختر المدربين',
      'slug_placeholder' => 'أدخل العلامة (slug) أو سيتم إنشاءها تلقائيًا.',
      'test' => 'اختبار',
      'timeline_description' => 'اسحب وقم بتغيير تسلسل الدروس/الاختبارات للدورة.',
      'title' => 'الدورات',
      'view' => 'عرض الدورات',
    ),
    'lessons' => 
    array (
      'fields' => 
      array (
        'slug' => 'العلامة (Slug)',
        'add_audio' => 'إضافة الصوت',
        'add_pdf' => 'إضافة PDF',
        'add_video' => 'أضف فيديو',
        'course' => 'دورة',
        'downloadable_files' => 'ملفات قابلة للتحميل',
        'free_lesson' => 'الدرس المجاني',
        'full_text' => 'نص كامل',
        'lesson_image' => 'صورة الدرس',
        'media_audio' => 'صوت الوسائط',
        'media_pdf' => 'قوات الدفاع الشعبي وسائل الإعلام',
        'media_video' => 'فيديو وسائط',
        'position' => 'موضع',
        'published' => 'نشرت',
        'short_text' => 'نص قصير',
        'title' => 'عنوان',
        'youtube_videos' => 'يوتيوب فيديو',
        'add_subtitle' => 'أضف عنوانًا فرعيًا',
      ),
      'create' => 'إنشاء الدرس',
      'edit' => 'تحرير الدرس',
      'enter_video_embed_code' => 'أدخل رمز تضمين الفيديو',
      'enter_video_url' => 'أدخل بيانات الفيديو',
      'max_file_size' => '(حجم الملف الأقصى 5 ميجابايت)',
      'remove' => 'إزالة',
      'select_course' => 'اختر الدورة',
      'short_description_placeholder' => 'إدخال وصف قصير للدرس',
      'slug_placeholder' => 'العلامة (Slug) الإدخال أو سيتم إنشاؤه تلقائيا',
      'title' => 'الدروس',
      'video_guide' => '<p class = "mb-1"> <b> Youtube: </b> اذهب إلى Youtube -> اذهب إلى الفيديو الذي تريد عرضه -> انقر على زر المشاركة أسفل الفيديو. انسخ تلك الروابط والصقها في مربع النص أعلاه </p>
<p class = "mb-1"> <b> Vimeo: </b> اذهب إلى Vimeo -> اذهب إلى الفيديو الذي تريد عرضه -> انقر على زر المشاركة وانسخ عنوان url للفيديو هنا </p>
<p class = "mb-1"> <b> تحميل: </b> تحميل ملف <b> mp4 </b> في إدخال الملف </p>
<p class = "mb-1"> <b> تضمين: </b> نسخ / لصق رمز التضمين في مربع النص أعلاه </p>',
      'view' => 'عرض الدروس',
      'vimeo_note' => '<b> تعليمات لإضافة رابط فيديو: </b> <br> انتقل إلى vimeo -> فيديو مفتوح -> انقر بزر الماوس الأيمن على الفيديو و <b> نسخ عنوان URL للفيديو </b> والصقه هنا. <br> لإضافة مقاطع فيديو متعددة ، قم بفصلها باستخدام علامة <b> و </b> (فاصلة).',
      'yt_note' => '<b> تعليمات لإضافة رابط فيديو: </b> <br> انتقل إلى youtube -> open video -> انقر بزر الماوس الأيمن على الفيديو و <b> نسخ عنوان URL للفيديو </b> والصقه هنا. <br> لإضافة مقاطع فيديو متعددة ، قم بفصلها باستخدام علامة <b> و </b> (فاصلة).',
    ),
    'pages' => 
    array (
      'fields' => 
      array (
        'slug' => 'العلامة (Slug)',
        'status' => 'الحالة',
        'clear' => 'واضح',
        'content' => 'يحتوى',
        'created' => 'انشاءت',
        'created_at' => 'تم إنشاؤها على',
        'drafted' => 'صاغت',
        'featured_image' => 'صورة مميزة',
        'meta_description' => 'ميتا الوصف',
        'meta_keywords' => 'كلمات دلالية',
        'meta_title' => 'عنوان الفوقية',
        'published' => 'نشرت',
        'title' => 'عنوان',
      ),
      'create' => 'إنشاء صفحة',
      'edit' => 'تعديل الصفحة',
      'max_file_size' => '(الحد الأقصى لحجم الملف 10 ميغابايت)',
      'title' => 'صفحات',
      'view' => 'عرض الصفحات',
    ),
    'faqs' => 
    array (
      'fields' => 
      array (
        'status' => 'الحالة',
        'answer' => 'إجابة',
        'category' => 'الفئة',
        'question' => 'سؤال',
      ),
      'create' => 'إنشاء التعليمات',
      'edit' => 'تحرير التعليمات',
      'title' => 'أسئلة وأجوبة',
      'view' => 'عرض الأسئلة الشائعة',
    ),
    'hero_slider' => 
    array (
      'fields' => 
      array (
        'status' => 'الحالة',
        'bg_image' => 'صورة BG',
        'buttons' => 
        array (
          'add' => 'إضافة',
          'label' => 'ضع الكلمة المناسبة',
          'link' => 'حلقة الوصل',
          'name' => 'زر',
          'note' => 'ملاحظة: يمكن إضافة 4 أزرار كحد أقصى. يرجى إضافة التصنيف والرابط للزر لإعادة توجيه الإجراء عند النقر فوق الزر.',
          'placeholder' => 'إضافة عدد من الأزرار التي تريد إضافتها',
          'title' => 'وصفت',
        ),
        'hero_text' => 'نص البطل',
        'name' => 'الاسم',
        'overlay' => 
        array (
          'note' => 'إذا قمت بتشغيله. سيتم عرض تراكب أسود على صورتك. سيكون مفيدًا عندما لا تكون صورة BG أغمق أو لا تحتوي على تراكب',
          'title' => 'غطاء',
        ),
        'sequence' => 'تسلسل',
        'sub_text' => 'النص الفرعي',
        'widget' => 
        array (
          'countdown_timer' => 'العد التنازلي',
          'input_date_time' => 'تاريخ الإدخال والوقت',
          'search_bar' => 'شريط البحث',
          'select_widget' => 'حدد القطعة',
          'title' => 'القطعة',
        ),
      ),
      'create' => 'إنشاء شريحة',
      'edit' => 'تحرير الشرائح',
      'manage_sequence' => 'إدارة تسلسل الشرائح',
      'note' => 'ملاحظة: يرجى تحميل .jpg أو .png بدقة <b> 1920 × 900 </b> للحصول على أفضل نتيجة استخدم صور أغمق أو متراكبة للحصول على نتيجة أفضل.',
      'off' => 'إيقاف',
      'on' => 'على',
      'save_sequence' => 'حفظ التسلسل',
      'sequence_note' => 'سحب وتغيير تسلسل الشريحة',
      'title' => 'بطل المتزلج',
      'view' => 'عرض الشرائح',
    ),
    'reasons' => 
    array (
      'fields' => 
      array (
        'status' => 'الحالة',
        'content' => 'يحتوى',
        'icon' => 'أيقونة',
        'title' => 'عنوان',
      ),
      'create' => 'انشاء السبب',
      'edit' => 'تحرير السبب',
      'note' => 'سيتم عرض الأسباب كمنزلق على الصفحة الرئيسية أدناه',
      'title' => 'أسباب',
      'view' => 'عرض الأسباب',
    ),
    'sponsors' => 
    array (
      'fields' => 
      array (
        'status' => 'الحالة',
        'link' => 'حلقة الوصل',
        'logo' => 'شعار',
        'name' => 'الاسم',
      ),
      'create' => 'إنشاء الرعاة',
      'edit' => 'تحرير الرعاة',
      'title' => 'الرعاة',
      'view' => 'عرض الرعاة',
    ),
    'teachers' => 
    array (
      'fields' => 
      array (
        'status' => 'الحالة',
        'email' => 'عنوان بريد الكتروني',
        'first_name' => 'الاسم الاول',
        'image' => 'صورة',
        'last_name' => 'الكنية',
        'password' => 'كلمه السر',
      ),
      'create' => 'انشاء المدرب',
      'edit' => 'تحرير المدرب',
      'title' => 'المدربون',
      'view' => 'عرض المدربين',
    ),
    'testimonials' => 
    array (
      'fields' => 
      array (
        'status' => 'الحالة',
        'content' => 'يحتوى',
        'name' => 'الاسم',
        'occupation' => 'الاحتلال',
      ),
      'create' => 'إنشاء التزكية',
      'edit' => 'تحرير التزكية',
      'title' => 'الشهادات - التوصيات',
      'view' => 'عرض الشهادات',
    ),
    'contacts' => 
    array (
      'fields' => 
      array (
        'email' => 'البريد الإلكتروني',
        'message' => 'رسالة',
        'name' => 'الاسم',
        'phone' => 'هاتف',
        'time' => 'الوقت',
      ),
      'title' => 'العملاء المحتملين',
    ),
    'dashboard' => 
    array (
      'amount' => 'السعر',
      'buy_course_now' => 'شراء الدورة الآن',
      'completed' => 'اكتمل',
      'course' => 'دورة',
      'courses' => 'الدورات',
      'email' => 'البريد الإلكتروني',
      'message' => 'رسالة',
      'message_by' => 'رسالة بواسطة',
      'name' => 'الاسم',
      'no_data' => 'لا تتوافر بيانات',
      'ordered_by' => 'ترتيب حسب',
      'recent_contact_requests' => 'طلبات الاتصال الحديثة',
      'recent_contacts' => 'الاتصالات الأخيرة',
      'recent_messages' => 'الرسائل الحديثة',
      'recent_orders' => 'الطلبيات الأخيرة',
      'recent_reviews' => 'المراجعات الأخيرة',
      'review' => 'مراجعة',
      'students' => 'الطلاب',
      'students_enrolled' => 'الطلاب المسجلين في دوراتك',
      'teachers' => 'مدربين',
      'time' => 'الوقت',
      'title' => 'لوحة التحكم',
      'trending' => 'الشائع',
      'view' => 'عرض',
      'view_all' => 'عرض الكل',
      'your_courses' => 'الدورات الخاصة بك',
      'my_course_bundles' => 'حزم دوراتي',
      'my_courses' => 'دوراتي',
      'course_and_bundles' => 'الدورات والحزم',
      'your_courses_and_bundles' => 'الدورات والحزم الخاصة بك',
      'pending' => 'قيد الانتظار',
      'pending_orders' => 'الطلبات معلقة',
      'success' => 'نجاح',
      'failed' => 'فشل',
      'my_subscribed_course_bundles' => 'حزم الدورات المشترك فيها',
      'my_subscribed_courses' => 'الدورات التي اشتركت فيها',
    ),
    'general_settings' => 
    array (
      'custom_css' => 'لغة تنسيق ويبCSS',
      'custom_js' => 'JS مخصص',
      'app_locale' => 'لغة التطبيق',
      'app_name' => 'اسم التطبيق',
      'app_timezone' => 'المنطقة الزمنية',
      'app_url' => 'عنوان URL',
      'captcha' => 'سواء كان التسجيل - تسجيل الدخول - اتصال captcha قيد التشغيل أو إيقاف التشغيل',
      'captcha_note' => 'كيفية الحصول على بيانات اعتماد Google reCaptcha؟',
      'captcha_site_key' => 'مفتاح Captcha',
      'captcha_site_secret' => 'كلمة Captcha السرية',
      'captcha_status' => 'حالة Captcha',
      'change_email' => 'تغيير البريد الالكتروني',
      'confirm_email' => 'تأكيد عنوان البريد الإلكتروني',
      'contact' => 
      array (
        'location_on_map' => 'الموقع على الخريطة',
        'map_note' => '<ol class="map-guide">
<li>اذهب إلى <a class="text-bold" target="_blank" href="//maps.google.com">خرائط جوجل</a> </li>
<li>ابحث في المكان الذي تريد إضافته عن طريق إدخال العنوان / الموقع في مربع الإدخال الموجود في الركن الأيمن العلوي</li>
<li>بمجرد الحصول على المكان الذي تريده. هذا يظهر التفاصيل في الشريط الجانبي الأيسر. انقر فوق <i class = "fa fa-share-alt text-primary"> زر </i></li>
<li>ستظهر نافذة منبثقة تحتوي على علامتي تبويب <b> إرسال رابط </b> و <b> تضمين خريطة </b></li>
<li>انقر فوق <b> تضمين خريطة </b> ستظهر لك اختيار "مكان على الخريطة"</li>
<li>الآن انقر على القائمة المنسدلة من اليسار. بشكل افتراضي ، يتم تحديد <b> متوسط </b>. انقر عليها واختر <b> كبير </b></li>
<li>الآن انقر على رابط <b class = "text-primary"> COPY HTML </b> منه و <b> لصق </b> هذا الرمز هنا في <b> الموقع على الخريطة </b>.</li>
 </ol>',
        'primary_address' => 'العنوان الرئيسي',
        'primary_email' => 'البريد الإلكتروني الرئيسي',
        'primary_email_note' => 'سيتم استخدام هذا البريد الإلكتروني لمطابقة رسائل البريد الإلكتروني "اتصل بنا"',
        'primary_phone' => 'الهاتف الرئيسي',
        'secondary_address' => 'العنوان الثانوي',
        'secondary_email' => 'البريد الإلكتروني الثانوي',
        'secondary_phone' => 'الهاتف الثانوي',
        'short_text' => 'نص قصير',
        'show' => 'اظهار',
        'title' => 'اتصل',
      ),
      'counter' => 'العداد',
      'counter_note' => '<b> ثابت </b> = إضافة بيانات لعداد يدويًا. الرجاء إدخال النص الدقيق الذي تريد عرضه في قسم عداد الواجهة الأمامية ، <br> <b> قاعدة البيانات / حقيقي </b> = سيستغرق الأمر بيانات حقيقية من قاعدة البيانات لجميع الحقول (الطلاب المسجلين ، إجمالي الدورات ، إجمالي المدربين)',
      'database' => 'قاعدة البيانات / حقيقي',
      'email' => 
      array (
        'mail_driver' => 'مشغل البريد',
        'mail_driver_note' => 'يمكنك تحديد أي برنامج تشغيل تريده لإعداد Mail. <ب> السابقين. SMTP و Mailgun و Mandrill و SparkPost و Amazon SES إلخ. </b> <br> أضف <b> برنامج تشغيل فرديًا فقط </b>',
        'mail_encryption' => 'تشفير البريد',
        'mail_encryption_note' => 'استخدم <b> tls </b> إذا كان موقعك يستخدم بروتوكول <b> HTTP </b> و <b>ssl </b> إذا كان موقعك يستخدم بروتوكول <b> HTTPS </b>',
        'mail_from_address' => 'البريد من العنوان',
        'mail_from_address_note' => 'سيتم استخدام هذا البريد الإلكتروني لمراسلات "نموذج الاتصال".',
        'mail_from_name' => 'البريد من الاسم',
        'mail_from_name_note' => 'سيكون هذا اسم العرض للبريد الإلكتروني المرسل.',
        'mail_host' => 'مضيف البريد',
        'mail_password' => 'بريد كلمة المرور',
        'mail_password_note' => 'أضف كلمة مرور البريد الإلكتروني التي تريد تكوينها لإرسال رسائل البريد الإلكتروني',
        'mail_port' => 'منفذ البريد',
        'mail_username' => 'اسم المستخدم',
        'mail_username_note' => 'أضف معرف بريدك الإلكتروني الذي تريد تكوينه لإرسال رسائل البريد الإلكتروني',
        'note' => '<b> ملاحظة مهمة </b>: إذا كنت تستخدم <b> GMAIL </b> لتكوين البريد ، فتأكد من إكمال العملية التالية قبل التحديث:
  <UL>
<li> انتقل إلى <a target="_blank" href="https://myaccount.google.com/security"> حسابي </a> من حسابك في Google الذي تريد تكوينه وتسجيل الدخول </li>
<li> قم بالتمرير لأسفل إلى <b> وصول أقل أمانًا للتطبيق </b> وتعيينه <b>ON</b> </li>
</UL>',
        'title' => 'اعدادات البريد',
      ),
      'enable_registration' => 'تمكين التسجيل',
      'font_color' => 'لون الخط',
      'footer' => 
      array (
        'bottom_footer' => 'تذييل اسفل',
        'bottom_footer_note' => 'ملاحظة: يتضمن النص حقوق الطبع والنشر وصلات تذييل',
        'copyright_text' => 'نص حقوق النشر',
        'custom_links' => 'روابط مخصصة',
        'featured_courses' => 'دورات مميزة',
        'footer_links' => 'روابط تذييل الصفحة',
        'link' => 'حلقة الوصل',
        'link_label' => 'ضع الكلمة المناسبة',
        'link_url' => 'URL',
        'newsletter_form' => 'نموذج النشرة الإخبارية',
        'popular_categories' => 'فئات مفضلة',
        'popular_courses' => 'دورات مفضلة',
        'recent_news' => 'أخبار حديثة',
        'section_1' => 'القسم 1',
        'section_2' => 'القسم 2',
        'section_3' => 'القسم 3',
        'short_description' => 'وصف قصير',
        'social_links' => 'روابط اجتماعية',
        'social_links_note' => 'أضف عنوان URL للرابط الاجتماعي وحدد أيقونة لتلك الوسائط من iconpicker. انقر فوق الزر <b> ADD + </b>. وسيتم إنشاء الرابط الاجتماعي الخاص بك. يمكنك أيضًا حذفها بالنقر فوق الزر <b> <i class = "fa fa-times"> </i> </b>',
        'title' => 'تذييل',
        'trending_courses' => 'الدورات الشائعة',
      ),
      'general' => 
      array (
        'title' => 'عام',
      ),
      'google_analytics' => 'Google Analytics Code',
      'google_analytics_id' => 'Google Analytics ID',
      'google_analytics_id_note' => 'كيف تحصل على Google Analytics ID؟',
      'homepage' => 'اختر الصفحة الرئيسية',
      'language_settings' => 
      array (
        'default_language' => 'اللغة الافتراضية',
        'display_type' => 'نوع العرض',
        'left_to_right' => 'من اليسار إلى اليمين',
        'right_to_left' => 'من اليمين الى اليسار',
        'title' => 'اعدادات اللغة',
      ),
      'layout' => 
      array (
        'title' => 'نسق',
      ),
      'layout_note' => 'سيؤدي هذا إلى تغيير تخطيط سمة الواجهة الأمامية',
      'layout_type' => 'نوع التخطيط',
      'logos' => 
      array (
        'title' => 'الشعارات',
      ),
      'mail_configuration_note' => 'هل قمت بتكوين: ربط إعدادات البريد </a>؟ من الضروري الإعداد لإرسال / استقبال رسائل البريد الإلكتروني',
      'mail_driver' => 'مشغل البريد',
      'mail_from_address' => 'عنوان البريد المرسل من',
      'mail_from_name' => 'اسم البريد المرسل من',
      'mail_host' => 'مضيف البريد',
      'mail_password' => 'كلمة المرور البريد',
      'mail_port' => 'منفذ البريد',
      'mail_username' => 'اسم المستخدم',
      'management' => 'الاعدادات العامة',
      'newsletter' => 
      array (
        'api_key' => 'مفتاح API',
        'api_key_note' => 'أنشئ <b> مفتاح API </b> من <a target="_blank" href="https://mailchimp.com/"> <b> حساب Mailchimp </b> </a> والصقه أعلاه مربع النوع.',
        'api_key_note_sendgrid' => 'قم بإنشاء <b> مفتاح API </b> من <a target="_blank" href="https://sendgrid.com/"> <b> حساب SendGrid </b> </a> والصقه في أعلاه مربع الكتابة.',
        'api_key_question' => 'كيفية توليد مفتاح API؟',
        'create_new' => 'إنشاء واختيار جديد',
        'double_opt_in' => 'Double Opt-in',
        'double_opt_in_note' => '<b> On </b> = سيُطلب من المستخدم في البريد الاشتراك في الاشتراك. <b> متوقف </b> = سيتم اشتراك المستخدم مباشرةً في الرسائل الإخبارية',
        'get_lists' => 'الحصول على قوائم',
        'list_id' => 'معرف القائمة',
        'list_id_note' => 'ابحث عن <b> معرف القائمة </b> والصقه في المربع أعلاه',
        'list_id_question' => 'كيفية العثور على معرف قائمة من Mailchimp؟',
        'list_id_question_sendgrid' => 'إنشاء قائمة بريد إلكتروني جديدة لـ SendGrid هنا.',
        'mail_provider' => 'مزود خدمة البريد',
        'mail_provider_note' => '<b> ملاحظة </b>: يمكنك تحديد أي مزود خدمة بريد لتلقي جميع رسائل البريد الإلكتروني المستخدمة في <b> الاشتراك في الرسالة الإخبارية </b>. <br> حدد وإعداد وفقًا للخطوات الموضحة. <b> إنه إلزامي </b> ، إذا كنت تريد استخدام نموذج <b> اشتراك النشرة الإخبارية </b>.',
        'mailchimp' => 'Mailchimp',
        'manage_lists' => 'إدارة قوائم SendGrid',
        'select_list' => 'اختر قائمة',
        'sendgrid' => 'SendGrid',
        'sendgrid_lists' => 'SendGrid قوائم البريد الإلكتروني',
        'title' => 'اعدادات النشرة الإخبارية',
      ),
      'password_expires_days' => 'صلاحية كلمة المرور',
      'password_history' => 'تاريخ كلمات المرور السابقة',
      'payment_settings' => 
      array (
        'client_id' => 'معرف العميل',
        'client_secret' => 'سر',
        'how_to_paypal' => 'كيفية الحصول على بيانات اعتماد API في PayPal؟',
        'how_to_stripe' => 'كيفية الحصول على أوراق اعتماد API STRIPE؟',
        'key' => 'مفتاح API',
        'live' => 'مباشر',
        'mode' => 'الوضع',
        'mode_note' => '<b> Sandbox </b> = سيتم استخدامه لاختبار المدفوعات باستخدام بيانات اعتماد اختبار PayPal. يمكن للحساب باستخدام USD فقط إجراء الدفعات مع PayPal في الوقت الحالي. سيتم إعادة توجيه هذه الخيارات لاختبار مدفوعات PayPal باستخدام بيانات اعتماد مستخدم Sandbox. سيتم استخدامه للمعاملات الوهمية فقط. <br>
<b> Live </b> = سيتم استخدام بيانات اعتماد Live PayPal معك لإجراء معاملات فعلية مع المستخدمين العاديين باستخدام حساب PayPal.',
        'offline_mode' => 'طريقة الدفع دون اتصال',
        'offline_mode_note' => 'يحصل المستخدم على مساعدة للدفع في وضع عدم الاتصال بالإنترنت عبر المشرف',
        'paypal' => 'طريقة الدفع باي بال',
        'paypal_note' => 'يعيد توجيه إلى paypal للدفع',
        'sandbox' => 'Sandbox',
        'secret' => 'API Secret',
        'stripe' => 'طريقة الدفع Stripe',
        'stripe_note' => 'تمكن المدفوعات في الموقع مع بطاقات الخصم / الائتمان',
        'title' => 'اعدادات الدفع',
        'select_currency' => 'اختر العملة',
        'bluesnap' => 'Bluesnap طريقة الدفع',
        'bluesnap_note' => 'نموذج بطاقة Bluesnap للدفع',
        'how_to_bluesnap' => 'كيفية الحصول على بيانات اعتماد Bluesnap API؟',
        'offline_mode_instruction' => 'أدخل تعليمات الدفع في وضع عدم الاتصال',
        'cashfree' => 'طريقة الدفع CashFree',
        'cashfree_app_id' => 'معرف التطبيق',
        'cashfree_mode_note' => '<b>رمل (Sandbox)</b> = سيتم استخدامها لاختبار عمليات الدفع باستخدام بيانات اختبار CashFree. يمكن للحسابات ذات العملة الهندية فقط إجراء مدفوعات باستخدام Cashfree حاليًا. ستقوم هذه الخيارات بتوجيهك إلى اختبار دفع Cashfree باستخدام بيانات اعتماد Sandbox. ستستخدم للمعاملات الوهمية فقط.<br><a href="//test.cashfree.com">كيفية الحصول على بيانات اختبار واجهة برمجة التطبيقات (API) لـ Cashfree؟</a><br>
<b>مباشر (Live)</b> = ستستخدم مع بيانات اعتماد Cashfree الحية لإجراء معاملة فعلية مع المستخدمين العاديين الذين يملكون حساب Cashfree. <br><strong>إذا قمت بتعيين بوابة الدفع هذه ، فقم بتعيين عملتك كـ INR</strong><br><a href="//www.cashfree.com">كيفية الحصول على بيانات واجهة برمجة التطبيقات (API) لـ Cashfree الحية؟</a>',
        'cashfree_note' => 'يتم توجيه الى CashFree للدفع',
        'cashfree_secret' => 'مفتاح السر',
        'flutter' => 'طريقة الدفع Flutter',
        'flutter_note' => 'يتم توجيه الى Flutter للدفع',
        'how_to_flutter' => 'كيفية الحصول على بيانات واجهة برمجة التطبيقات (API) لـ Flutter؟',
        'how_to_payu' => 'كيفية الحصول على بيانات اعتماد API في  PayUMoney ؟',
        'how_to_razorpay' => 'كيفية الحصول على بيانات اعتماد API في  RAZORPAY ؟',
        'instamojo' => 'طريقة الدفع Instamojo',
        'instamojo_mode_note' => '<b>(Sandbox)</b> = سيتم استخدامها لاختبار عمليات الدفع باستخدام بيانات اختبار Instamojo. ستقوم هذه الخيارات بتوجيهك إلى اختبار دفع Instamojo باستخدام بيانات اعتماد Sandbox. ستستخدم للمعاملات الوهمية فقط.<br><a href="//test.instamojo.com/">كيفية الحصول على بيانات اختبار واجهة برمجة التطبيقات (API) لـ Instamojo؟</a><br>
<b>مباشر (Live)</b> = ستستخدم مع بيانات اعتماد Instamojo الحية لإجراء معاملة فعلية مع المستخدمين العاديين الذين يملكون حساب Instamojo. <br><a href="//www.instamojo.com">كيفية الحصول على بيانات واجهة برمجة التطبيقات (API) لـ Instamojo الحية؟</a>',
        'instamojo_note' => 'يتم توجيه الى instamojo للدفع',
        'instamojo_token' => 'API Token',
        'payu_mode_note' => '<b>Sandbox</b> = سيُستخدم لاختبار عمليات الدفع باستخدام بيانات اعتماد اختبار PayUMoney. يمكن للحسابات التي تعمل بالروبية الهندية (INR) فقط إجراء عمليات دفع باستخدام هذا الخيار. سيقوم هذا الخيار بتوجيهك إلى اختبار دفع PayUMoney باستخدام بيانات اعتماد مستخدم الاختبار. سيتم استخدامه فقط للمعاملات التجريبية.<br>
<b>مباشر (Live)</b> = سيتم استخدامه مع بيانات اعتماد PayUMoney الفعلية لإجراء معاملات فعلية مع المستخدمين العاديين الذين يمتلكون حساب PayUMoney.<br><strong>إذا قمت بتعيين بوابة الدفع هذه، فيرجى تعيين عملتك إلى INR</strong>',
        'payu_note' => 'اعادة التوجيه إلى PayUMoney للدفع',
        'payu_salt' => 'Salt',
        'razorpay' => 'طريقة الدفع Razorpay',
        'razorpay_note' => 'اعادة التوجيه إلى RazorPay للدفع',
      ),
      'requires_approval' => 'يتطلب موافقة',
      'retest_note' => 'تمكين / تعطيل إذا كان المستخدم سيكون قادرا على إعادة اختبار للامتحان نفسه',
      'sections_note' => 'بمجرد النقر فوق "تحديث" ، سترى قائمة بالأقسام لتشغيل / إيقاف',
      'static' => 'ثابتة',
      'theme_layout' => 'تخطيط الموضوع',
      'title' => 'عام',
      'total_courses' => 'أدخل مجموع الدورات. على سبيل المثال: 1 كيلو ، 1000 إلخ',
      'total_students' => 'أدخل مجموع الطلاب. على سبيل المثال: 1 كيلو ، مليون ، 1000 إلخ.',
      'total_teachers' => 'أدخل مجموع المدربين. على سبيل المثال: 1 كيلو ، 1000 إلخ',
      'user_registration_settings' => 
      array (
        'desc' => 'سيتم عرض الحقول المحددة من الشريط الجانبي الأيمن في نموذج التسجيل',
        'fields' => 
        array (
          'address' => 'عنوان',
          'city' => 'مدينة',
          'country' => 'بلد',
          'dob' => 'تاريخ الولادة',
          'email' => 'البريد الإلكتروني',
          'female' => 'انثى',
          'first_name' => 'الاسم الاول',
          'gender' => 'جنس',
          'last_name' => 'الكنية',
          'male' => 'ذكر',
          'other' => 'آخر',
          'password' => 'كلمه السر',
          'phone' => 'هاتف',
          'pincode' => 'الرقم السري',
          'state' => 'حالة',
        ),
        'title' => 'إعدادات تسجيل المستخدم',
      ),
      'lesson_note' => 'تمكين / تعطيل إذا كان المستخدم سيكون قادرا على تخطي قبل انتهاء الموقت.',
      'troubleshoot' => 'استكشاف',
      'show_offers_note' => 'تمكين / تعطيل إذا عرضت الكوبون رابط الصفحة ليتم عرضها في أسفل الصفحة',
      'how_to_onesignal' => 'كيفية إنشاء التطبيق في OneSignal؟',
      'onesignal_note' => 'تمكين / تعطيل  OneSignal.',
      'setup_onesignal' => 'كيفية إعداد OneSignal؟',
      'api_clients' => 
      array (
        'api_client_name' => 'اسم عميل API',
        'enable' => 'تمكين',
        'fields' => 
        array (
          'action' => 'اجراء',
          'id' => 'رقم الهوية',
          'name' => 'الاسم',
          'secret' => 'سر',
          'status' => 'الحالة',
        ),
        'generate' => 'توليد',
        'live' => 'مباشر',
        'please_input_api_client_name' => 'الرجاء إدخال اسم عميل API',
        'revoke' => 'الغاء',
        'revoked' => 'لاغي',
        'something_went_wrong' => 'هناك خطأ ما',
        'title' => 'عملاء API',
        'developer_manual' => 'دليل المطور',
        'note' => '<span class="text-danger font-weight-bold">ملحوظة:</span> سيكون هذا القسم مفيد لأغراض واجهة برمجة التطبيقات (API). يمكنك إنشاء <b> (Client ID)</b> و<b> (Client Secret)</b> لاستخدامها في تطبيق Android الخاص بك أو في Postman لاختبار الواجهة البرمجية. <br> يجب عليك تخصيص المدربات وعنوان URL وفقًا لاحتياجاتك.',
      ),
      'teacher_commission_rate' => 'معدل عمولة المدرب',
      'box' => 'صندوق',
      'layout_label' => 'نسق',
      'layout_type_note' => 'سيؤدي هذا إلى تغيير نوع تخطيط الواجهة الأمامية',
      'list_update_note' => 'بمجرد النقر فوق "تحديث" ، سترى قائمة بالأقسام لتشغيل / إيقاف.',
      'wide' => 'واسع',
      'admin_order_mail' => 'بريد الطلب',
      'admin_order_mail_note' => 'تمكين / تعطيل إذا كان المشرف سيكون قادرًا على استلام رسائل الطلب الجديدة',
      'admin_registration_mail' => 'بريد التسجيل',
      'admin_registration_mail_note' => 'تمكين / تعطيل إذا كان المشرف سيكون قادرًا على استلام رسائل تسجيل المستخدم الجديد',
    ),
    'invoices' => 
    array (
      'fields' => 
      array (
        'amount' => 'كمية',
        'download' => 'تحميل',
        'order_date' => 'تاريخ الطلب',
        'view' => 'رأي',
      ),
      'title' => 'الفواتير',
    ),
    'logo' => 
    array (
      'favicon' => 'اضافة Facicon',
      'favicon_note' => 'ملاحظة: قم بتحميل الشعار بدقة <b> 32 × 32 </b> بكسل وإضافات <b> .png </b> أو <b> .gif </b> أو <b> .ico </b>',
      'logo_b' => 'الشعار 1',
      'logo_b_note' => 'ملاحظة: تحميل الشعار مع <b> نص أسود وخلفية شفافة بتنسيق .png </b> و <b> 294 × 50 </b> (WxH) بكسل. <br> <b> الارتفاع </b> <b> العرض </b> وفقًا لـ <b> نسبة العرض إلى الارتفاع </b>.',
      'logo_popup' => 'شعار للنوافذ المنبثقة',
      'logo_popup_note' => 'ملاحظة: أضف دقة شعار مربعة أقل من <b> 72 × 72 </b> بكسل',
      'logo_w' => 'الشعار 2',
      'logo_w_note' => 'ملاحظة: تحميل الشعار مع <> نص أبيض وخلفية شفافة بتنس> و <b> 294 × 50 </b> (WxH) بكسل. <br> <b> الارتفاع </b> <b> العرض </b> وفقًا لـ <b> نسبة العرض إلى الارتفاع </b>.',
      'logo_white' => 'الشعار 3',
      'logo_white_note' => 'ملاحظة: قم بتحميل الشعار مع <b> بنص أبيض وخلفية شفافة فقط بتنسيق .png </b> و <b> 294 × 50 </b> (WxH>) بكسل. <br> <b> الارتفاع </b> ثابت ، <b> العرض </b> وفقًا لـ <b> نسبة العرض إلى الارتفاع </b>.',
      'title' => 'تغيير الشعار',
    ),
    'menu-manager' => 
    array (
      'title' => 'مدير القائمة',
      'button' => 
      array (
        'add_to_menu' => 'اضف الي القائمة',
        'update_sort_order' => 'تحديث الترتيب',
        'view_details' => 'عرض التفاصيل',
      ),
      'custom_links_card_header_text' => 'روابط مخصصة',
      'header_text' => 'هيكل القائمة',
      'instruction_text' => 'ضع كل عنصر بالترتيب الذي تفضله. انقر على السهم الموجود على يمين العنصر لعرض المزيد من الخيارات.',
      'label' => 
      array (
        'label' => 'ملصق',
        'url' => 'URL',
      ),
      'pages_card_header_text' => 'الصفحات',
    ),
    'messages' => 
    array (
      'compose' => 'مؤلف موسيقى',
      'search_user' => 'بحث المستخدم',
      'select_recipients' => 'اختر المستلمين',
      'start_conversation' => 'بدء محادثة',
      'title' => 'الرسائل',
      'type_a_message' => 'اكتب رسالة',
    ),
    'orders' => 
    array (
      'complete' => 'أكمل يدويا',
      'download_invoice' => 'تحميل فاتورة',
      'fields' => 
      array (
        'amount' => 'كمية',
        'date' => 'تاريخ الطلب',
        'items' => 'العناصر',
        'ordered_by' => 'مرتبة حسب',
        'payment_status' => 
        array (
          'completed' => 'منجز',
          'failed' => 'فشل',
          'pending' => 'قيد الانتظار',
          'title' => 'دفع',
        ),
        'payment_type' => 
        array (
          'offline' => 'غير متصل على الانترنت',
          'paypal' => 'باي بال',
          'stripe' => 'بطاقة الائتمان / الخصم (بوابة الدفع عبر الشريط)',
          'title' => 'نوع الدفع',
        ),
        'reference_no' => 'رقم المرجع.',
        'user_email' => 'البريد الالكتروني للمستخدم',
      ),
      'offline_requests' => 'طلبات غير متصل',
      'title' => 'الطلبات',
      'view_invoice' => 'عرض الفاتورة',
    ),
    'questions' => 
    array (
      'create' => 'إنشاء سؤال',
      'edit' => 'تحرير السؤال',
      'fields' => 
      array (
        'correct' => 'صيح',
        'course' => 'دورة',
        'lesson' => 'درس',
        'option_text' => 'نص الخيار',
        'question' => 'سؤال',
        'question_image' => 'صورة السؤال',
        'score' => 'أحرز هدفا',
        'tests' => 'اختبارات',
        'title' => 'عنوان',
        'option_explanation' => 'شرح الخيار',
      ),
      'title' => 'الأسئلة',
      'view' => 'عرض الأسئلة',
      'test' => 'اختبار',
    ),
    'questions_options' => 
    array (
      'create' => 'إنشاء خيار',
      'edit' => 'تحرير الخيار',
      'fields' => 
      array (
        'correct' => 'صيح',
        'course' => 'دورة',
        'lesson' => 'درس',
        'option_text' => 'نص الخيار',
        'question' => 'سؤال',
        'question_option' => 'خيار السؤال',
        'score' => 'أحرز هدفا',
        'tests' => 'اختبارات',
        'title' => 'عنوان',
      ),
      'title' => 'خيار الأسئلة',
      'view' => 'عرض خيارات السؤال',
    ),
    'reviews' => 
    array (
      'fields' => 
      array (
        'content' => 'محتوي',
        'course' => 'دورة',
        'time' => 'الوقت',
        'user' => 'المستخدم',
      ),
      'title' => 'التعليقات',
    ),
    'social_settings' => 
    array (
      'fb_api_note' => 'كيفية الحصول على مؤهلات اعتماد واجهة برمجة تطبيقات Facebook؟',
      'fb_note' => 'تمكين / تعطيل تسجيل الدخول إلى موقع Facebook',
      'google_api_note' => 'كيفية الحصول على بيانات اعتماد Google API؟',
      'google_note' => 'تمكين / تعطيل تسجيل الدخول إلى Google لموقع الويب',
      'linkedin_api_note' => 'كيفية الحصول على مؤهلات LinkedIn API؟',
      'linkedin_note' => 'تمكين / تعطيل تسجيل الدخول إلى LinkedIn لموقع الويب',
      'management' => 'الوضع الاجتماعي',
      'twitter_api_note' => 'كيفية الحصول على بيانات اعتماد Twitter API؟',
      'twitter_note' => 'تمكين / تعطيل تسجيل الدخول إلى Twitter لموقع الويب',
    ),
    'tests' => 
    array (
      'create' => 'إنشاء اختبار',
      'edit' => 'تحرير الاختبار',
      'fields' => 
      array (
        'course' => 'دورة',
        'description' => 'وصف',
        'lesson' => 'درس',
        'published' => 'نشرت',
        'questions' => 'الأسئلة',
        'title' => 'عنوان',
        'score_field' => 'درجة النجاح',
        'score_field_placeholder' => 'اقل درجة للنجاح',
      ),
      'title' => 'اختبارات',
      'view' => 'عرض الاختبارات',
    ),
    'translations' => 
    array (
      'add_new_locale' => 'إضافة لغة جديدة',
      'adding' => 'جار الاضافة ...',
      'append_new_translations' => 'إلحاق ترجمات جديدة',
      'changed' => 'تغيير',
      'choose_a_group' => 'اختر مجموعة لعرض ترجمات المجموعة. إذا لم تكن هناك مجموعات مرئية ، فتأكد
                                 لقد قمت بتشغيل عمليات الترحيل واستوردت الترجمات.',
      'current_supported_locales' => 'اللغات المدعومة الحالية',
      'done_importing' => 'تم الاستيراد والمعالجة <strong class = "counter"> N </strong>! إعادة تحميل هذه الصفحة إلى
                             تحديث المجموعات!',
      'done_publishing_for_all_groups' => 'تم نشر الترجمات لكل المجموعة!',
      'done_publishing_for_group' => 'تم نشر الترجمات للمجموعة',
      'done_searching' => 'تم البحث عن الترجمات ، التي عثر عليها <strong class = "counter"> N </strong>!',
      'enter_new_locale_key' => 'أدخل مفتاح اللغة الجديد',
      'export_all_translations' => 'تصدير جميع الترجمات',
      'import_groups' => 'استيراد المجموعات',
      'import_groups_note' => '<p> سيؤدي هذا إلى الحصول على جميع ملفات الإعدادات المحلية من مجلد <code> lang </code> وإدراجها في قاعدة البيانات. <br> <b> إلحاق ترجمات جديدة: </b> سيُلحق فقط الملفات والبيانات الجديدة <b> & </b>
                                             <b> استبدل الترجمات الحالية: </b> ستحل محل السجلات الحالية وفقًا للملفات </p>',
      'key' => 'مفتاح',
      'publish_all' => 'نشر الكل',
      'publish_all_warning' => 'هل أنت متأكد أنك تريد نشر كل مجموعة الترجمات؟ سيؤدي هذا إلى الكتابة فوق ملفات اللغة الحالية.',
      'publish_translations' => 'نشر الترجمات',
      'publishing' => 'جار النشر...',
      'replace_existing_translations' => 'استبدال الترجمات الحالية',
      'supported_locales' => 'اللغات المدعومة',
      'title' => 'مدير الترجمة',
      'total' => 'مجموع',
      'translation_warning' => 'هل أنت متأكد أنك تريد نشر مجموعة الترجمات: group؟ سيؤدي هذا إلى الكتابة فوق ملفات اللغة الحالية',
      'warning' => 'تحذير ، الترجمات غير مرئية حتى يتم تصديرها مرة أخرى إلى ملف التطبيق / اللغة ، باستخدام
                         <code> php translation translation: export </code> زر الأمر أو النشر.',
    ),
    'update' => 
    array (
      'current_version' => 'النسخة الحالية',
      'file_replaced' => 'سيتم تحديث / استبدال الملفات التالية',
      'note_before_upload' => '<p> <b> يرجى أخذ BACKUP قبل التحديث. </b> قد يأتي ملف zip المحدث مع مجلدات جديدة وتحديثات للملفات. <b> سيتم استبدال ملفاتك الحالية بملف جديد </b>. لذا ، <b> إذا قمت بإجراء أي تغييرات في ملفات التطبيق الحالية ، فسيكون هذا المفقود </b>. </p>
                         <p> إذا كنت تقوم بالتحميل مباشرة من مربع إدخال الملف أدناه ، فسيتم استبدال ملفاتك. نوصي بشدة بالقيام بذلك الاستبدال اليدوي للملفات واحدًا تلو الآخر أو تعديل التغييرات من خلال مقارنة الملف الذي تم تعديله الحالي والملفات الجديدة المحدثة. </p>
                         <p> إذا كنت لا تزال تعاني من الارتباك. يرجى الاتصال بنا ، وسوف نوجهك لتحديث التطبيق الخاص بك </p>',
      'note_before_upload_title' => 'قراءة الملاحظات التالية قبل التحديث',
      'title' => 'تحديث الموضوع',
      'upload' => 'تحميل الإصدار الجديد <small> (update.zip) </small>',
      'warning' => '<b> تحذير: نوصيك بشدة بتحديث المظهر حسب رقم الإصدار </b>. <br> <b> مثال: </b> update_v1.zip، update_v2.zip. من فضلك لا تقفز رقم الإصدار. إذا كان رقم الإصدار هو 1 وتريد تحديثه ، فقم بالتحديث إلى الإصدار 2. لا تقم بالتحديث مباشرة إلى الإصدار 3.',
    ),
    'certificates' => 
    array (
      'download' => 'تحميل',
      'fields' => 
      array (
        'action' => 'اجراء',
        'course_name' => 'اسم الدورة',
        'progress' => 'تقدم',
      ),
      'title' => 'شهادات',
      'view' => 'عرض',
    ),
    'bundles' => 
    array (
      'fields' => 
      array (
        'slug' => 'العلامة (Slug)',
        'status' => 'الحالة',
        'category' => 'الفئة',
        'course' => 'دورة',
        'course_image' => 'صورة الدورة',
        'description' => 'وصف',
        'featured' => 'متميز',
        'lessons' => 
        array (
          'add' => 'أضف دروس',
          'view' => 'عرض الدروس',
        ),
        'meta_description' => 'الوصف (Meta Description)',
        'meta_keywords' => 'الكلمات الدلالية (Meta Keywords)',
        'meta_title' => 'العنوان (Meta Title)',
        'popular' => 'مفضل',
        'price' => 'السعر',
        'published' => 'منشور',
        'sidebar' => 'أضف الشريط الجانبي',
        'start_date' => 'تاريخ البدء',
        'teachers' => 'مدربين',
        'title' => 'عنوان',
        'trending' => 'الشائع',
        'courses' => 'الدورات',
        'free' => 'مجاناً',
        'unpublished' => 'غير منشور',
        'expire_at' => 'تاريخ انتهاء الحزمة/تعطيلها',
      ),
      'add_categories' => 'إضافة فئات',
      'add_teachers' => 'إضافة المدربين',
      'category' => 'الفئة',
      'course_timeline' => 'الجدول الزمني للدورة',
      'create' => 'إنشاء حزمة',
      'edit' => 'تحرير الحزمة',
      'lesson' => 'درس',
      'listing_note' => 'سيتم عرض وترتيب الدروس المنشورة فقط.',
      'save_timeline' => 'حفظ الجدول الزمني',
      'select_category' => 'اختر الفئة',
      'select_teachers' => 'اختر المدربين',
      'slug_placeholder' => 'أدخل العلامة (Slug) أو سيتم إنشاءها تلقائيًا.',
      'test' => 'اختبار',
      'timeline_description' => 'اسحب وقم بتغيير تسلسل الدروس/الاختبارات للدورة.',
      'title' => 'الحزم',
      'view' => 'عرض الحزم',
      'add_courses' => 'إضافة الدورات',
      'select_courses' => 'اختر الدورات',
    ),
    'reports' => 
    array (
      'bundles' => 'الحزم',
      'courses' => 'الدورات',
      'fields' => 
      array (
        'bundle' => 'حزمة',
        'completed' => 'مكتمل',
        'content' => 'يحتوى',
        'course' => 'دورة',
        'earnings' => 'أرباح',
        'name' => 'الاسم',
        'orders' => 'الطلبات',
        'students' => 'الطلاب',
        'time' => 'زمن',
        'user' => 'المستعمل',
        'amount' => 'الكمية',
        'date' => 'التاريخ',
        'student' => 'الطالب',
        'subscribed_name' => 'اسم الاشتراك',
        'transaction' => 'رقم المعاملة',
      ),
      'sales_report' => 'تقرير المبيعات',
      'students_report' => 'تقرير الطلاب',
      'title' => 'التقارير',
      'total_earnings' => 'الأرباح الكلية',
      'total_sales' => 'إجمالي المبيعات',
      'apply_date' => 'تطبيق التاريخ',
      'date_input_lang' => 
      array (
        'last_30_days' => 'اخر 30 يوم',
        'last_7_days' => 'اخر 7 ايام',
        'last_month' => 'الشهر الماضي',
        'quarter_to_date' => 'منذ ثلاثة اشهر',
        'this_month' => 'هذا الشهر',
        'today' => 'اليوم',
        'year_to_date' => 'السنه الماضية',
        'yesterday' => 'البارحة',
      ),
      'date_range' => 'نطاق التاريخ',
      'filter' => 'تصفية',
      'reset' => 'اعادة الضبط',
      'select_bundle' => 'اختر الحزمة',
      'select_course' => 'اختر الدورة',
      'select_student' => 'اختر الطالب',
      'subscribe_earning' => 'الربح من الاشتراك',
      'subscribed_plan' => 'الخطة المشترك بها',
      'total_subscribe' => 'اجمالي الاشتراك',
    ),
    'coupons' => 
    array (
      'amount_note' => 'إذا تم اختيار <b>معدل الخصم</b>، أدخل نسبة الخصم. إذا تم اختيار <b>معدل ثابت</b>، أدخل المبلغ المحدد الذي سيتم خصمه.',
      'bundles' => 'الحزم',
      'courses' => 'الدورات',
      'create' => 'إنشاء القسيمة',
      'description' => 'وصف',
      'discount_rate' => 'معدل الخصم',
      'edit' => 'تحرير القسيمة',
      'fields' => 
      array (
        'amount' => 'المبلغ',
        'code' => 'الشفرة',
        'description' => 'وصف',
        'expires_at' => 'تنتهي في',
        'for' => 'الي',
        'min_price' => 'الحد الأدنى لسعر الطلب',
        'name' => 'الاسم',
        'per_user_limit' => 'الحد  لكل مستخدم',
        'status' => 'الحالة',
        'total' => 'مجموع',
        'type' => 'نوع',
      ),
      'flat_rate' => 'معدل ثابت',
      'off' => 'إيقاف',
      'on' => 'تشغيل',
      'per_user_limit_note' => 'حدد كم مرة يمكن لمستخدم واحد استخدام هذا الكوبون. بشكل افتراضي، يمكن استخدامه مرة واحدة.',
      'title' => 'القسائم',
      'total_note' => 'عدد القسائم التي ستصدر',
      'type_note' => '<b>معدل الخصم (%) :</b> إذا قمت باختيار هذا، سيتم تطبيق نسبة خصم على إجمالي الشراء. مثلاً، السعر = 100 دولار ونسبة الخصم 10٪، سيتم خصم 10٪ من 100 دولار.<br><b>معدل ثابت :</b> إذا قمت باختيار هذا، سيتم خصم مبلغ معين من إجمالي الشراء. مثلاً، السعر = 100 دولار والمعدل الثابت هو 25 دولار، سيتم خصم 25 دولار من 100 دولار.',
      'unlimited' => 'غير محدود',
      'view' => 'عرض الالقسائم',
    ),
    'sitemap' => 
    array (
      'daily' => 'اليومي',
      'generate' => 'توفير',
      'generated' => 'تم إنشاء ملف Sitemap بنجاح.',
      'monthly' => 'شهريا',
      'records_note' => 'عدد السجلات لكل ملف.',
      'records_per_file' => 'السجلات لكل ملف',
      'sitemap_note' => 'ستنشئ أداة خريطة الموقع هذه ملف Sitemap للدورة  المنشورة ، والحزم والمدونات.',
      'title' => 'خريطة الموقع',
      'weekly' => 'أسبوعي',
    ),
    'tax' => 
    array (
      'create' => 'إنشاء ضريبة',
      'edit' => 'تحرير الضريبة',
      'fields' => 
      array (
        'name' => 'الاسم',
        'rate' => 'معدل',
        'status' => 'الحالة',
      ),
      'off' => 'إيقاف',
      'on' => 'على',
      'title' => 'ضريبة',
      'view' => 'عرض الضريبة',
    ),
    'forum_category' => 
    array (
      'create' => 'إنشاء فئة المنتدى',
      'edit' => 'تحرير فئة المنتدى',
      'fields' => 
      array (
        'category' => 'الفئة',
        'color' => 'اللون',
        'order' => 'طلب',
        'parent_category' => 'القسم الرئيسي',
        'status' => 'الحالة',
      ),
      'off' => 'إيقاف',
      'on' => 'تشغيل',
      'title' => 'فئات المنتدى',
      'view' => 'مشاهدة قسم المنتدى',
    ),
    'payments' => 
    array (
      'title' => 'المدفوعات',
      'total_earnings' => 'الأرباح الكلية',
      'total_withdrawals' => 'إجمالي السحوبات',
      'total_withdrawal_pending' => 'إجمالي السحب معلق',
      'total_balance' => 'الرصيد الكلي',
      'earnings' => 'أرباح',
      'withdrawals' => 'السحب',
      'add_withdrawal_request' => 'إضافة طلب السحب',
      'approve' => 'يوافق',
      'reject' => 'رفض',
      'payment_type' => 
      array (
        'bank' => 'مصرف',
        'paypal' => 'باي بال',
        'offline' => 'غير متصل على الانترنت',
      ),
      'status' => 
      array (
        'pending' => 'قيد الانتظار',
        'approved' => 'وافق',
        'rejected' => 'مرفوض',
      ),
      'fields' => 
      array (
        'amount' => 'كمية',
        'payment_type' => 'نوع الدفع',
        'status' => 'الحالة',
        'remarks' => 'ملاحظات',
        'date' => 'تاريخ',
        'teacher_name' => 'اسم المدرب',
      ),
    ),
    'live_lesson_slots' => 
    array (
      'closed' => 'مغلق',
      'create' => 'إنشاء الدرس المباشر',
      'edit' => 'تعديل الدرس المباشر',
      'fields' => 
      array (
        'change_default_setting' => 'تغيير الإعدادات الافتراضية',
        'date' => 'تاريخ',
        'date_of_slot' => 'تاريخ',
        'duration' => 'المدة (بالدقائق)',
        'lesson' => 'الدرس',
        'meeting_id' => 'معرف (ID) الاجتماع',
        'password' => 'كلمة المرور',
        'short_text' => 'فرز الوصف',
        'student_limit' => 'حد الطلاب',
        'topic' => 'الموضوع',
      ),
      'select_lesson' => 'اختر الدرس',
      'short_description_placeholder' => 'الوصف المختصر',
      'slot' => 'مكان',
      'slot_booked_student_list' => 'قائمة الطلاب الذين حجزوا مكانًا',
      'start_url' => 'بداية URL',
      'student_email' => 'البريد الالكتروني للطالب',
      'student_name' => 'اسم الطالب',
      'title' => 'اماكن الدروس المباشرة',
      'type' => 
      array (
        'daily' => 'يومي',
        'monthly' => 'شهري',
        'select_type' => 'اختر النوع',
        'weekly' => 'اسبوعي',
      ),
      'view' => 'عرض المكان',
    ),
    'live_lessons' => 
    array (
      'create' => 'انشاء درس مباشر',
      'edit' => 'تعديل درس مباشر',
      'fields' => 
      array (
        'course' => 'دورة',
        'short_text' => 'الوصف',
        'title' => 'العنوان',
      ),
      'select_course' => 'اختر الدورة',
      'short_description_placeholder' => 'الوصف المختصر للدرس المباشر',
      'title' => 'الدرس المباشر',
      'view' => 'عرض الدرس',
    ),
    'stripe' => 
    array (
      'plan' => 
      array (
        'bundle' => '{0} وصول غير محدود|{99} لا يوجد وصول|[1,*] :quantity',
        'course' => '{0} وصول غير محدود|{99} لا يوجد وصول|[1,*] :quantity',
        'create' => 'انشاء خطة Stripe',
        'edit' => 'تعديل خطة Stripe',
        'fields' => 
        array (
          'amount' => 'الكمية',
          'bundle' => 'عدد الحزم',
          'course' => 'عدد الدورات',
          'currency' => 'العملة',
          'description' => 'الوصف',
          'expire' => 'الانتهاء',
          'interval' => 'الفاصل الزمني',
          'name' => 'الاسم',
          'product' => 'المنتج',
          'select_bundles' => 'اختر الحزمة',
          'select_course' => 'اختر الدورة',
        ),
        'input_quantity_note' => '"0 يعني وصولًا غير محدود، 99 يعني عدم الوصول"',
        'interval_type' => 
        array (
          '' => 'حدد 	الفاصل الزمني',
          'day' => 'يوم',
          'month' => 'شهر',
          'week' => 'اسبوع',
          'year' => 'سنه',
        ),
        'select_product' => 'حدد المنتج',
        'stripe_currency_note' => '<strong>يرجى استخدام رمز العملة بالحروف الصغيرة. يمكنك العثور على رمز العملة الخاص بك <a href="//stripe.com/docs/currencies#presentment-currencies" target="_blank">هنا</a></strong>',
        'stripe_expire_note' => 'يرجى إدخال تاريخ انتهاء الصلاحية على سبيل المثال: 1=أيام، 1=شهر، 1=أسبوع، 1=سنة',
        'title' => 'خطط Stripe',
        'view' => 'عرض خطط Stripe',
      ),
    ),
    'subscription' => 
    array (
      'active_plan' => 'تفعيل خطة',
      'bundle' => 'حزمة',
      'cancel_title' => 'الغاء الاشتراك',
      'click_here' => 'اضغط هنا',
      'course' => 'الدورة',
      'date' => 'التاريخ',
      'download' => 'تحميل',
      'invoice_list' => 'قائمة الفواتير',
      'quantity' => '{0} غير محدود|{99} غير متاح|[1,*] :quantity',
      'sub_total' => 'المجموع الفرعي',
      'subscribe_plan' => 'خطة الاشتراك',
      'title' => 'الاشتراك',
      'total' => 'الاجمالي',
    ),
    'wishlist' => 
    array (
      'course' => 'الدورة',
      'title' => 'قائمة المفضلة',
    ),
    'zoom_setting' => 
    array (
      'audio_options' => 
      array (
        'both' => 'كلاهما',
        'telephony' => 'الاتصالات',
        'voip' => 'VoIP',
      ),
      'auto_recording_options' => 
      array (
        'cloud' => 'سحابي',
        'local' => 'محلي',
        'none' => 'لا يوجد',
      ),
      'fields' => 
      array (
        'api_key' => 'مفتاح API',
        'api_secret' => 'مفتاح السر API',
        'audio_option' => 'خيارات الصوت',
        'auto_recording' => 'التسجيل التلقائي',
        'host_video' => 'استضافة الفيديو',
        'join_before_host' => 'الانضمام قبل المضيف',
        'meeting_join_approval' => 'الموافقة على الانضمام إلى الاجتماع',
        'participant_mic_mute' => 'كتم صوت الميكروفون',
        'timezone' => 'المنطقة الزمنية',
        'waiting_room' => 'غرفة الانتظار',
        'participant_video' => 'فيديو المشارك',
      ),
      'management' => 'اعدادت ZOOM',
      'meeting_approval_options' => 
      array (
        'automatically' => 'تلقائي',
        'manually' => 'يدوي',
        'no_registration_required' => 'لا يتطلب التسجيل',
      ),
    ),
  ),
  'lang' => 
  array (
    'sp' => 'الأسبانية',
    'fr' => 'الفرنسية',
    'en' => 'الإنجليزية',
    'ar' => 'عربى',
  ),
  'general' => 
  array (
    'yes' => 'نعم فعلا',
    'trash' => 'قمامة، يدمر، يهدم',
    'toolbar_btn_groups' => 'شريط الأدوات مع مجموعات الأزرار',
    'toggle_navigation' => 'تبديل الملاحة',
    'sr_no' => 'الأب رقم.',
    'show' => 'تبين',
    'read_more' => 'قراءة المزيد',
    'none' => 'لا شيء',
    'no_data_available' => 'لا تتوافر بيانات',
    'no' => 'لا',
    'more' => 'أكثر من',
    'actions' => 'أفعال',
    'active' => 'نشيط',
    'all' => 'الكل',
    'back' => 'الى الخلف',
    'buttons' => 
    array (
      'cancel' => 'إلغاء',
      'save' => 'حفظ',
      'update' => 'تحديث',
    ),
    'copyright' => 'حقوق النشر',
    'create_new' => 'انشاء جديد إبداع جديد',
    'custom' => 'العادة',
    'delete' => 'حذف',
    'delete_selected' => 'احذف المختار',
    'edit' => 'تصحيح',
    'hide' => 'إخفاء',
    'inactive' => 'غير نشط',
    'id' => 'معرف ID',
  ),
  'frontend' => 
  array (
    'auth' => 
    array (
      'login_box_title' => 'تسجيل الدخول',
      'login_button' => 'تسجيل الدخول',
      'login_with' => 'تسجيل الدخول مع :social_media',
      'register_box_title' => 'تسجيل',
      'register_button' => 'تسجيل',
      'remember_me' => 'تذكرنى',
    ),
    'badges' => 
    array (
      'trending' => 'الشائع',
      'popular' => 'المفضل',
      'featured' => 'المميز',
    ),
    'blog' => 
    array (
      'add_comment' => 'أضف تعليق',
      'blog_categories' => 'المدونة <span> الفئات. </span>',
      'by' => 'بواسطة',
      'featured_course' => 'دورات مميزة',
      'login_to_post_comment' => 'تسجيل الدخول لنشر تعليق',
      'no_comments_yet' => 'لا تعليقات حتى الآن ، كن أول من يعلق.',
      'popular_tags' => '<span> العلامات الشائعة. </span>',
      'post_comments' => 'نشر <span> تعليقات. </span>',
      'related_news' => '<span> ذات صلة </span> الأخبار',
      'search_blog' => 'بحث المدونة',
      'share_this_news' => 'شارك هذا الخبر',
      'title' => 'مدونة',
      'write_a_comment' => 'أكتب تعليقا',
    ),
    'cart' => 
    array (
      'card_number' => 'رقم البطاقة',
      'card_number_placeholder' => 'أدخل رقم بطاقتك',
      'cart' => 'عربة التسوق',
      'checkout' => 'الدفع',
      'complete_your_purchases' => 'أكمل <span> مشترياتك. </span>',
      'confirmation_note' => 'بتأكيد هذا الشراء ، أوافق على <b> شروط الاستخدام ، وسياسة رد الأموال </b> و <b> سياسة الخصوصية </b>',
      'course_name' => 'اسم الدورة ',
      'course_type' => 'دورة كتابية',
      'cvv' => 'CVV',
      'empty_cart' => 'عربة التسوق فارغة',
      'expiration_date' => 'تاريخ إنتهاء الصلاحية',
      'go_back_to_cart' => 'العودة إلى السلة',
      'mm' => 'MM',
      'name_on_card' => 'الاسم على البطاقة',
      'name_on_card_placeholder' => 'أدخل الاسم المكتوب على بطاقتك',
      'no_payment_method' => 'لا توجد طريقة دفع متاحة في هذه اللحظة',
      'offline_payment' => 'غير متصل الدفع',
      'offline_payment_note' => ' سيقوم المسؤولون التنفيذيون لدينا بالاتصال بك وإعطائك تعليمات بشأن الدفع وشراء الدورة .',
      'order_detail' => 'ترتيب <span> التفاصيل. </span>',
      'order_item' => 'طلب <span> عنصر. </span>',
      'order_payment' => 'اطلب <span> الدفع. </span>',
      'pay_now' => 'ادفع الآن',
      'pay_securely_paypal' => 'ادفع بأمان باستخدام PayPal',
      'payment_cards' => 'بطاقة الائتمان أو الخصم',
      'payment_status' => 'حالة السداد',
      'paypal' => 'باي بال',
      'request_assistance' => 'طلب المساعدة',
      'see_more_courses' => 'رؤية المزيد من الدورات',
      'starts' => 'يبدأ',
      'stripe_error_message' => 'يرجى تصحيح الأخطاء والمحاولة مرة أخرى.',
      'success_message' => 'تهانينا. استمتع بدراستك',
      'total' => 'مجموع',
      'your_payment_status' => '<span> حالة الدفع الخاصة بك. </span>',
      'your_shopping_cart' => 'سلة التسوق الخاصة بك',
      'yy' => 'YY',
      'product_name' => 'اسم المنتج',
      'product_type' => 'نوع المنتج',
      'connection_timeout' => 'انتهى وقت محاولة الاتصال',
      'offline_request' => 'تم استلام الطلب بنجاح! تحقق من بريدك الإلكتروني المسجل لمزيد من التفاصيل.',
      'payment_done' => 'تم الدفع بنجاح!',
      'payment_failed' => 'خطأ! عملية الدفع فشلت!',
      'product_added' => 'تم اضافة المنتج الى السلة بنجاح',
      'try_again' => 'خطأ! حاول مرة اخرى.',
      'unknown_error' => 'حدث خطأ غير معروف',
      'purchase_successful' => 'تهانينا! لقد اشتريت هذه الدورة.',
      'amount' => 'كمية',
      'apply' => 'تطبيق',
      'empty_input' => 'اكتب رمز القسيمة قبل التقديم',
      'invalid_coupon' => 'قسيمة غير صالحة!',
      'item' => 'بند',
      'items' => 'العناصر',
      'offers' => 'عروض',
      'price' => 'السعر',
      'sub_total' => 'المجموع الفرعي',
      'total_payable' => 'مجموع المستحق',
      'cashfree' => 'CashFree',
      'flutter' => 'Flutter',
      'instamojo' => 'Instamojo',
      'pay_securely_cashfree' => 'ادفع بأمان مع CashFree',
      'pay_securely_flutter' => 'ادفع بأمان مع Flutter',
      'pay_securely_instamojo' => 'ادفع بأمان مع Instamojo',
      'pay_securely_razorpay' => 'ادفع بأمان مع RazorPay',
      'pay_securely_payu' => 'ادفع بأمان مع PayUMoney',
      'payu' => 'PayUMoney',
      'razorpay' => 'RazorPay',
      'user_email' => 'البريد الاكتروني',
      'user_name' => 'الاسم',
      'user_phone' => 'الهاتف',
    ),
    'contact' => 
    array (
      'box_title' => 'اتصل بنا',
      'button' => 'إرسال المعلومات',
      'keep_in_touch' => 'حافظ على <span> في اللمس. </span>',
      'message' => 'رسالة',
      'phone_number' => 'رقم الهاتف',
      'send_email' => 'ارسل بريد الكتروني',
      'send_message_now' => 'إرسال رسالة الآن',
      'send_us_a_message' => 'أرسل لنا <span>رسالة</span>',
      'title' => 'اتصل',
      'your_email' => 'بريدك الالكتروني',
      'your_name' => 'اسمك',
    ),
    'course' => 
    array (
      'add_review_now' => 'إضافة مراجعة الآن',
      'add_reviews' => 'أضف <span> مراجعات. </span>',
      'add_to_cart' => 'أضف إلى السلة',
      'added_to_cart' => 'يضاف إلى العربة',
      'author' => 'مؤلف',
      'average_rating' => 'متوسط تقييم',
      'buy_note' => 'يمكن للطلاب فقط شراء الدورة',
      'buy_now' => 'اشتري الآن',
      'by' => 'بواسطة',
      'category' => 'الفئة',
      'chapter_videos' => 'الفصل فيديو',
      'chapters' => 'فصول',
      'completed' => 'مكتمل',
      'continue_course' => 'مواصلة الدورة',
      'course_detail' => 'تفاصيل الدورة',
      'course_details' => '<span> تفاصيل الدورة. </span>',
      'course_name' => 'اسم الدورة ',
      'course_reviews' => 'المراجعات',
      'course_timeline' => 'للدورة  <b> الجدول الزمني: </b>',
      'course_type' => 'دورة كتابية',
      'courses' => 'الدورات',
      'details' => 'تفاصيل',
      'download_files' => 'تحميل ملفات',
      'enrolled' => 'المقيدين',
      'featured' => 'متميز',
      'featured_course' => '<span> دورة مميزة </span>',
      'find_courses' => 'البحث عن الدورات',
      'find_your_course' => 'ابحث عن دورة .',
      'full_text' => 'نص كامل',
      'give_test_again' => 'إعطاء اختبار مرة أخرى',
      'go' => 'اذهب',
      'mb' => 'MB',
      'message' => 'رسالة',
      'next' => 'التالى',
      'no_reviews_yet' => 'لا توجد تعليقات حتى الآن.',
      'none' => 'لا شيء',
      'popular' => 'مفضل',
      'prev' => 'السابق',
      'price' => 'السعر',
      'progress' => 'تقدم',
      'ratings' => 'تصنيفات',
      'recent_news' => '<span> الأخيرة </span> الأخبار.',
      'sort_by' => '<b> ترتيب </b> حسب',
      'stars' => 'نجوم',
      'starts' => 'يبدأ',
      'students' => 'الطلاب',
      'submit_results' => 'إرسال النتائج',
      'test' => 'اختبار',
      'title' => 'دورة',
      'trending' => 'الشائع',
      'view_all_news' => 'عرض جميع الأخبار',
      'your_rating' => 'تقييمك',
      'your_test_score' => 'نقاط الاختبار الخاصة بك',
      'certified' => 'أنت معتمد لهذه الدورة',
      'finish_course' => 'الانتهاء من الدورة',
      'course' => 'دورة',
      'bundle_detail' => 'تفاصيل الحزمة',
      'bundle_reviews' => 'مراجعات الحزمة',
      'bundles' => 'الحزم',
      'available_in_bundles' => 'متاح أيضا في حزم',
      'complete_test' => 'الرجاء إكمال الاختبار',
      'looking_for' => 'البحث عن؟',
      'get_now' => 'احصل الان',
      'explanation' => 'الشرح',
      'not_attempted' => 'لم يحاول',
      'find_your_bundle' => 'البحث عن حزمة',
      'select_category' => 'اختر الفئة',
      'original_price' => 'السعر الاصلي',
      'no_lessons' => 'لا يوجد دروس',
      'available_slots' => 'الاماكن المتاحة',
      'book_slot' => 'احجز مكان',
      'date' => 'التاريخ',
      'full_slot' => 'الاماكن ممتلئة',
      'live_lesson_join_url' => 'الانضمام إلى URL',
      'live_lesson' => 'الدروس المباشرة',
      'live_lesson_meeting_date' => 'تاريخ الاجتماع',
      'live_lesson_meeting_duration' => 'الفترات',
      'live_lesson_meeting_id' => 'ID الاجتماع',
      'live_lesson_meeting_password' => 'كلمة المرور',
      'slot' => 'مكان',
      'subscribe' => 'اشتراك',
      'your_result' => 'نتيجتك',
      'your_score' => 'نسبة التقدم',
    ),
    'faq' => 
    array (
      'contact_us' => 'اتصل بنا',
      'find' => 'ابحث عن <span> أسئلتك وإجاباتك. </span>',
      'make_question' => 'كون سؤالا',
      'title' => 'بشكل متكرر <span> الأسئلة المتداولة </span>',
    ),
    'home' => 
    array (
      'all_teachers' => 'جميع المدربين',
      'learn_new_skills' => 'تعلم مهارات جديدة',
      'online_available_courses' => 'الدورات المتاحة على الانترنت',
      'our_professionals' => 'المهنيين لدينا',
      'popular_teachers' => '<span> شائع </span> المدربون',
      'search_course' => 'بحث دورة',
      'search_course_placeholder' => 'اكتب ماذا تريد أن تتعلم اليوم؟',
      'search_courses' => '<span> بحث </span> عن الدورات .',
      'students_enrolled' => 'الطلاب المسجلين',
      'teachers' => 'المدربون',
      'title' => 'الصفحة الرئيسية',
      'what_they_say_about_us' => 'ماذا يقولون عنا',
      'no_data_available' => 'لا تتوافر بيانات',
    ),
    'layouts' => 
    array (
      'partials' => 
      array (
        'advantages' => 'مزايا',
        'browse_course_by_category' => 'استعرض الدورات <span> حسب الفئة. </span>',
        'browse_featured_course' => 'استعرض <span> الدورة  المميزة. </span>',
        'call_us_registration' => 'اتصل بنا للحصول على تسجيل مجاني',
        'contact_us' => 'اتصل بنا',
        'course_detail' => 'تفاصيل الدورة',
        'courses_categories' => 'فئات الدورات',
        'days' => 'أيام',
        'email_address' => 'عنوان بريد الكتروني',
        'email_registration' => 'مراسلتنا عبر البريد الإلكتروني للحصول على تسجيل مجاني',
        'faq' => 'التعليمات',
        'faq_full' => 'بشكل متكرر <span> الأسئلة المتداولة </span>',
        'featured_course' => '<span> دورة مميزة </span>',
        'get_in_touch' => 'ابقى على تواصل',
        'hours' => 'ساعات',
        'latest_news_blog' => 'أحدث <span> مدونة الأخبار. </span>',
        'learn_new_skills' => 'تعلم مهارات جديدة',
        'minutes' => 'الدقائق',
        'popular_courses' => 'الدورات <span> الشائعة. </span>',
        'primary' => 'ابتدائي',
        'recent_news' => '<span> الأخيرة </span> الأخبار.',
        'search_courses' => 'البحث في الدورات',
        'search_our_courses' => 'بحث دوراتنا',
        'search_placeholder' => 'اكتب ماذا تريد أن تتعلم اليوم؟',
        'second' => 'ثانيا',
        'seconds' => 'ثواني',
        'social_network' => 'شبكة اجتماعية',
        'sponsors' => 'الرعاة.',
        'students' => 'الطلاب',
        'students_testimonial' => 'شهادات الطلاب',
        'subscribe_newsletter' => 'اشترك في النشرة الإخبارية',
        'subscribe_now' => 'إشترك الآن',
        'trending_courses' => 'الشائعة <span> الدورات . </span>',
        'view_all_news' => 'عرض جميع الأخبار',
        'view_all_popular_courses' => 'عرض جميع الدورات الشعبية',
        'view_all_trending_courses' => 'عرض جميع الدورات ',
        'why_choose' => 'اسباب اختيارنا',
        'certificate_verification' => 'التحقق من الشهادة',
        'more_faqs' => 'المزيد من الأسئلة الشائعة',
        'offers' => 'عروض',
      ),
    ),
    'modal' => 
    array (
      'already_user_note' => 'بالفعل مستخدم؟ تسجيل الدخول هنا',
      'login_now' => 'تسجيل الدخول الآن',
      'login_register' => '<a href="#" class="font-weight-bold go-login px-0">تسجيل الدخول</a>إلى موقعنا ، أو<a href="#" class="font-weight-bold go-register px-0" id="">تسجيل</a>',
      'my_account' => 'حسابي',
      'new_user_note' => 'مستخدم جديد؟ سجل هنا',
      'register_now' => 'سجل الان',
      'registration_message' => 'تم التسجيل بنجاح الرجاء تسجيل الدخول',
    ),
    'passwords' => 
    array (
      'expired_password_box_title' => 'انتهت كلمة المرور الخاصة بك.',
      'forgot_password' => 'نسيت رقمك السري؟',
      'reset_password_box_title' => 'إعادة تعيين كلمة المرور',
      'reset_password_button' => 'إعادة تعيين كلمة المرور',
      'send_password_reset_link_button' => 'إرسال رابط إعادة تعيين كلمة المرور',
      'update_password_button' => 'إرسال رابط إعادة تعيين كلمة المرور',
    ),
    'search_result' => 
    array (
      'blog' => 'مدونة',
      'course_detail' => 'تفاصيل الدورة',
      'course_name' => 'اسم الدورة ',
      'course_type' => 'نوع دورة',
      'featured' => 'متميز',
      'none' => 'لا شيء',
      'popular' => 'مفضل',
      'search_blog' => 'بحث المدونة',
      'sort_by' => '<b> ترتيب </b> حسب',
      'starts' => 'يبدأ',
      'students' => 'الطلاب',
      'trending' => 'الشائع',
    ),
    'teacher' => 
    array (
      'course_detail' => 'تفاصيل الدورة',
      'courses_by_teacher' => 'الدورات <span> حسب المدرب. </span>',
      'send_now' => 'ارسل الان',
      'students' => 'الطلاب',
      'title' => 'المدربون',
    ),
    'user' => 
    array (
      'passwords' => 
      array (
        'change' => 'تغ',
      ),
      'profile' => 
      array (
        'avatar' => 'الصورة الرمزية',
        'created_at' => 'أنشئت في',
        'edit_information' => 'تعديل المعلومات',
        'email' => 'البريد الإلكتروني',
        'first_name' => 'الاسم الاول',
        'last_name' => 'الكنية',
        'last_updated' => 'آخر تحديث',
        'name' => 'الاسم',
        'update_information' => 'تحديث المعلومات',
      ),
    ),
    'certificate_verification' => 
    array (
      'date_on_certificate' => 'تاريخ الشهادة. السابق. 2018-11-25',
      'name_on_certificate' => 'الاسم في الشهادة. السابق. يوحنا',
      'not_found' => 'لم يتم العثور على شهادة للمعلومات المقدمة.',
      'title' => 'التحقق من الشهادة',
      'verify_now' => 'تحقق الآن',
    ),
    'footer' => 
    array (
      'featured_courses' => 'دورات مميزة',
      'popular_categories' => 'فئات شعبية',
      'popular_courses' => 'دورات شعبية',
      'trending_courses' => 'تتجه الدورات',
      'useful_links' => 'روابط مفيدة',
    ),
    'offers' => 
    array (
      'minimum_order_amount' => 'الحد الأدنى للطلب',
      'no_offers' => 'لا عروض',
      'per_user' => 'لكل مستخدم',
      'title' => 'عروض',
      'unlimited' => 'غير محدود',
      'usage' => 'استعمال',
      'validity' => 'صلاحية',
    ),
    'subscription' => 
    array (
      'bundles' => 'الحزم',
      'courses' => 'الدورات',
    ),
  ),
  'teacher' => 
  array (
    'teacher_register' => 'سجل كمدرب؟ انقر هنا',
    'teacher_register_box_title' => 'سجل المدرب',
    'facebook_link' => 'رابط فيسبوك ',
    'twitter_link' => 'رابط تويتر',
    'linkedin_link' => 'رابط لينكدين',
    'payment_details' => 'بيانات الدفع',
    'bank' => 'البنك',
    'paypal' => 'باي بال',
    'paypal_email' => 'بريد باي بال',
    'bank_details' => 
    array (
      'name' => 'اسم البنك',
      'ifsc_code' => 'كود IFSC',
      'account' => 'رقم حساب',
      'holder_name' => 'أسم الحساب',
      'account_number' => 'رقم حساب',
      'bank_code' => 'رمز بنكي',
    ),
    'description' => 'وصف',
  ),
  'subscription' => 
  array (
    'already_subscribe' => 'مشترك بالفعل',
    'bundle' => '{0} إمكانية الوصول إلى باقات غير محدودة|{99} الباقة غير متاحة|[1,1]:quantity باقة|[2,*]:quantity باقات',
    'button' => 'اشتراك',
    'cancel' => 'تم إلغاء الاشتراك!',
    'course' => '{0} إمكانية الوصول إلى الدورة غير محدودة|{99} الدورة غير متاحة|[1,1]:quantity دورة |[2,*]:quantity دورات',
    'done' => 'تم الإشتراك بنجاح!',
    'failed' => 'خطأ! فشل الاشتراك!',
    'form' => 
    array (
      'address' => 'العنوان',
      'card' => 'بطاقة',
      'city' => 'المدينة',
      'country_code' => 'كود الدولة',
      'email' => 'البريد الالكتروني',
      'name' => 'الاسم',
      'state' => 'المقاطعة',
      'zip' => 'الرمز البريدي',
    ),
    'go_to_home' => 'الرئيسية',
    'go_to_plan' => 'انتقل إلى الخطط',
    'payment_status' => 'حالة دفع الاشتراك',
    'plan' => 'الخطة',
    'price' => 'السعر',
    'quantity' => '{0} إمكانية الوصول غير محدودة|[1,*] المجموع: الكمية :quantity الوصول',
    'resume' => 'استئناف الاشتراك!',
    'title' => 'خطة الاشتراك',
    'trial_period' => 'الفترة التجريبية :days ايام',
    'update' => 'تم تحديث الاشتراك بنجاح!',
    'your_subscription_status' => '<span> حالة الاشتراك</span>',
  ),
);

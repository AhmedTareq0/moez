...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        // Ensure we have a usable theme layout value (fall back to 1)
        $type = config('theme_layout') ?: 1;
        $sectionsRow = Config::where('key', '=', 'layout_' . $type)->first();
        // If there's no config entry in the database, fallback to the default sections
        // defined in config/sections.php. Ensure we decode to an object so the views
        // can access properties like $sections->search_section->status.
        if ($sectionsRow) {
            $sections = json_decode($sectionsRow->value);
        } else {
            $defaultSections = config('sections.layout_' . $type, []);
            $sections = json_decode(json_encode($defaultSections));
        }
        ...
        // If the roles haven't been seeded yet, Spatie will throw a RoleDoesNotExist exception.
        // Catch that and fall back to an empty collection so the front page still renders.
        try {
        ...
        if ((int)config('counter') == 1) {
            ...
            // Guard role queries in case roles haven't been created yet
            try {
        ...
        $layout = config('theme_layout') ?: 1;
        return view($this->path . '.index-' . $layout, compact('popular_courses', 'featured_courses', 'sponsors', 'total_students', 'total_courses', 'total_teachers', 'testimonials', 'news', 'trending_courses', 'teachers', 'faqs', 'course_categories', 'reasons', 'sections', 'categories'));
    }
    ...
    public function subscribe(Request $request)
    {
        ...
        // dd(Newsletter::subscribe('rincewind@discworld.com'));

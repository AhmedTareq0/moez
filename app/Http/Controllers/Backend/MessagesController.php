        ...
        try {
            ...
            foreach (auth()->user()->threads as $item) {
                if ($item->userUnreadMessagesCount(auth()->user()->id)) {
                    $messageModel = $item->messages()->orderBy('id', 'desc')->first();
                    $messageText = $messageModel ? str_limit($messageModel->body, 35) : '';
                    $participant = $item->participants()->with('user')->where('user_id', '<>', auth()->user()->id)->first();
                    $title = $participant && $participant->user ? $participant->user->name : '';
                    $data = [
                        'thread_id' => $item->id,
                        'message' => $messageText,
            ...
            // If the chat tables are not migrated or any DB issue, don't break the whole request.
            \Log::warning('Unread messages unavailable: ' . $e->getMessage());
